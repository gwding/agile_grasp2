...
Quadric::Quadric(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> >& T_cams,
  ...
  : sample_(sample), majority_cam_source_(majority_cam_source), is_deterministic_(false)
{
...
void Quadric::fitQuadric(const std::vector<int> &indices, bool forces_PSD)
{
	int n = indices.size();
  ...
  Eigen::MatrixXd eigen_vectors(10, 10);
  Eigen::VectorXd eigen_values(10);
  if (!forces_PSD)
  {
    Eigen::MatrixXd lambda;
    solveGeneralizedEigenProblem(M, N, eigen_vectors, lambda);
    eigen_values = lambda.col(0).cwiseQuotient(lambda.col(2));
  }
  else
  {
    N += (0.0000000001 * N.norm() * Eigen::MatrixXd::Identity(10,10));
    Eigen::GeneralizedSelfAdjointEigenSolver<Eigen::MatrixXd> es(M, N);
    eigen_values = es.eigenvalues();
    eigen_vectors = es.eigenvectors();
  }
  
  // extract quadric parameters from eigen decomposition
  int min_index;
...
void Quadric::findTaubinNormalAxis(const std::vector<int> &indices, const Eigen::MatrixXi& cam_source)
{
  ...
  Eigen::MatrixXi samples_cam_source;
  if (!is_deterministic_)
  {
    int num_samples = 50;
    samples.resize(3, num_samples);
    samples_cam_source.resize(cam_source.rows(), num_samples);
    if (indices.size() > num_samples)
    {
      for (int t = 0; t < num_samples; t++)
      {
        ...
        samples_cam_source.col(t) = cam_source.col(r);
      }
    }
    else
    {
  ...
  Eigen::VectorXd num_source(cam_source.rows());
  num_source.setZero();
  for (int samples_idx = 0; samples_idx < samples_cam_source.cols(); samples_idx++)
  {
    for (int cam_idx = 0; cam_idx < samples_cam_source.rows(); cam_idx++)
    {
      if (samples_cam_source(cam_idx, samples_idx) == 1)
        num_source(cam_idx)++;
    }
  }
